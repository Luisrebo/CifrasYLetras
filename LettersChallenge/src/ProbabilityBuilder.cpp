...
void ProbabilityBuilder::update(const std::string& palabraDelDiccionario) {
	...
	for (int i = 0; i < palabraDelDiccionario.length(); ++i) {
		...
		letterCounts[indiceCharEnArray].first += 1;
		letterCounts[indiceCharEnArray].second = palabraDelDiccionario[i];//redundante
...
void ProbabilityBuilder::calcularProbabilidades() {
	...
	for (int i = 0; i<probabilidades.matricesDeProbabilidades.probabilidadLetraPorNivel.size(); ++i) {
		...
		for (int j = 0; j < probabilidades.matricesDeProbabilidades.probabilidadLetraPorNivel[i].size(); ++j) {
			...
			probabilidades.matricesDeProbabilidades.probabilidadLetraPorNivel[i][j] =
				probabilidades.matricesDeProbabilidades.probabilidadLetraPorNivel[i][j] /
				std::max(evitarDivisionEntreCero, probabilidades.sumaTotalDeProbabilidadesPorFilasLetraPorNivel[i]);
		}
	}
	...
	for (int i = 0; i < probabilidades.matricesDeProbabilidades.probabilidadLetraPosterior.size(); ++i) {
		...
		for (int j = 0; j < probabilidades.matricesDeProbabilidades.probabilidadLetraPorNivel[i].size(); ++j) {
			...
			probabilidades.matricesDeProbabilidades.probabilidadLetraPosterior[i][j] =
				probabilidades.matricesDeProbabilidades.probabilidadLetraPosterior[i][j] /
				std::max(probabilidades.sumaTotalDeProbabilidadesPorFilasLetraPosterior[i], evitarDivisionEntreCero);
		}
	}
}

MatricesDeProbabilidades ProbabilityBuilder::getProbabilidades() {
