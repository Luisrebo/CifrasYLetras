...
void IHeuristic::preparar(Trie::Link nodo) {
...
int IHeuristic::prepararAlturasNodo(Trie::Link nodo) {
	...
	for (auto hijo : nodo->hijos)
		if (hijo)
			maxAlt = std::max(maxAlt, prepararAlturasNodo(hijo) + 1);
...
void IHeuristic::prepareTrieParaExplorarPorHeuristica(Trie::Link nodo) {
...
void IHeuristic::fillVectoresDeNodosSegunHeuristica(Trie::Link nodo){
	if (!nodo)
		return;
