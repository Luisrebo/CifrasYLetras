...
Trie::Trie() : raiz(new Trie::TreeNode(' ', 0)), heuristica(HeuristicFactory::crearHeuristica(TipoHeuristica::ALTURA)) {}
...
Trie::TreeNode* Trie::search(const std::string& palabraBuscada, Trie::Link nodo) { //Ruben me dijo que con tipos pequeños como int no usara& con string si? lo hace alberto en el tad
...
int Trie::inserta(const std::string& palabraOriginal, const std::string& palabraRaw, Link nodo) {
	...
	if (nodo->nivel == palabraRaw.size()) {
		...
		nodo->altura = std::max(inserta(palabraOriginal, palabraRaw, nuevoHijo) + 1, nodo->altura);// si en el nodo actual yo ya tenia un hijo mas profundo que el que acabo de expandir
...
void Trie::setHeuristica(TipoHeuristica tipo) {
	heuristica = HeuristicFactory::crearHeuristica(tipo, builderProbabilidades.getProbabilidades());
}
...
void Trie::resuelve(std::istream& archivo) {
	...
	//imprimirGlobalStatsResolutions(std::cout); movemos a CLI
}
...
void Trie::cargarDesdeArchivo(std::istream& archivo) {
	...
	std::string palabraOriginal, palabraRaw;
...
bool Trie::existe(const std::string& palabra, Link const& nodo) {
	return nodo != nullptr && nodo->nivel == palabra.size() && nodo->terminal == true;
}

void Trie::imprimirGlobalStatsResolutions(std::ostream& os) { promediosResolucionesLetras.imprimirResumen(os); }
